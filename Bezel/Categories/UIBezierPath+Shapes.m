...
+ (UIBezierPath *)pathFromString:(NSString *)string forSize:(CGSize)size
{
    ...
    NSRegularExpression *nameRegEx = [NSRegularExpression regularExpressionWithPattern:@".*(?==)" options: NSRegularExpressionCaseInsensitive error: nil];
    NSString *pname = [str substringWithRange:[nameRegEx firstMatchInString: str options: NSMatchingWithTransparentBounds range: NSMakeRange(0, 20.0)].range];
