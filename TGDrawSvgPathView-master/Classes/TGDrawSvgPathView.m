...
@interface TGDrawSvgPathView ()
{
    CAShapeLayer *scaledShapeLayer;
...
- (void)scale
{
    ...
    scaledPath = CGPathCreateCopyByTransformingPath(_shapeView.path,
                                                              &scaleTransform);
    
    // Create a new shape layer and assign the new path
    scaledShapeLayer = [CAShapeLayer layer];
    scaledShapeLayer.path = scaledPath;
    scaledShapeLayer.strokeColor = _strokeColor.CGColor;
    scaledShapeLayer.fillColor = [UIColor whiteColor].CGColor;
    ...
    [self performSelector:@selector(fillPathForLayer) withObject:nil afterDelay:0.5 * _animationDuration];
    CGPathRelease(scaledPath); // release the copied path

}

- (void)fillPathForLayer {
    CAShapeLayer *filledShapeLayer = [CAShapeLayer layer];
    filledShapeLayer.lineWidth = 1.0;
    filledShapeLayer.strokeColor = _strokeColor.CGColor;
    filledShapeLayer.fillColor = [UIColor blackColor].CGColor;
    filledShapeLayer.path = scaledPath;
    ...
    [filledShapeLayer addAnimation:drawAnimation forKey:@"StrokeAnimation"];
    [scaledShapeLayer addSublayer:filledShapeLayer];
}
